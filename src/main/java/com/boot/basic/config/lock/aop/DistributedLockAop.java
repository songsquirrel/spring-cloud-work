package com.boot.basic.config.lock.aop;

import com.boot.basic.constants.SymbolConstant;
import com.boot.basic.config.lock.annotation.DistributedLock;
import com.boot.basic.config.lock.annotation.LockKeyFields;
import com.boot.basic.enums.ApiResultEnum;
import com.boot.basic.exception.BizException;
import com.boot.basic.utils.SpelUtil;
...
@ComponentScan(excludeFilters = {
        @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = RedissonAutoConfiguration.class)
})
@ConditionalOnProperty(name = "distributed.lock.enable", havingValue = "true")
public class DistributedLockAop {
    ...
    @Pointcut("@annotation(com.song.basic_x.config.lock.annotation.DistributedLock)")
    ...
    public Object around(ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (parameters.length > 0) {
            for (int i = 0; i < parameters.length; i++) {
                if (parameters[i].isAnnotationPresent(LockKeyFields.class)) {
                    Map<String, Object> argLockFieldMap = new HashMap<>(3);
        ...
        try {
            ...
            throw new BizException(ApiResultEnum.FAILED);
        } finally {
            if (lock != null) {
                lock.forceUnlock();
            }
        ...
        if (!locked) {
            ...
            throw new BizException(ApiResultEnum.BUSINESS_BUSY);
        } else {
            return pjp.proceed();
        }
    }
